...
export default function ProgressPage() {
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      ...
      // Completed sessions with trainer info
  ...
  if (isLoading) {
    return (
      <div className="p-6 max-w-6xl mx-auto space-y-6">
        {/* Placeholder header */}
        <div className="h-8 bg-[#2d2d2d] rounded w-1/3 mb-6 animate-pulse"></div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {[...Array(4)].map((_, i) => (
            ...
            >
              <div className="h-6 bg-[#252525] rounded w-4/5 mb-2"></div>
  ...
  // Most booked trainer
  const trainerFrequency: Record<string, { count: number; trainer: EnrichedSession["trainer"] }> = {};
  sessions.forEach((s) => {
  ...
  const mostBooked = Object.values(trainerFrequency)
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold text-(--color-secondary)">Your Progress</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          { title: "Fitness Level", value: user.fitness_level || "—", icon: "📊" },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-(--color-accent) border border-[#333333] rounded-xl overflow-hidden">
          <div className="p-5 border-b border-[#333333]">
            <h2 className="text-lg font-semibold text-(--color-secondary)">Last Session</h2>
          </div>
          <div className="p-5">
            {lastSession ? (
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="font-bold text-(--color-secondary)">{lastSession.trainer.name}</h3>
                  <p className="text-(--color-contrast) text-sm">{lastSession.date}</p>
                </div>
              </div>
            ) : (
        ...
        <div className="bg-(--color-accent) border border-[#333333] rounded-xl overflow-hidden">
          <div className="p-5 border-b border-[#333333]">
            <h2 className="text-lg font-semibold text-(--color-secondary)">Most Booked Trainer</h2>
          </div>
          <div className="p-5">
            {mostBooked ? (
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="font-bold text-(--color-secondary)">{mostBooked.trainer.name}</h3>
                  <p className="text-(--color-contrast) text-sm">
                    {mostBooked.count} {mostBooked.count === 1 ? "session" : "sessions"} together
                  </p>
                </div>
              </div>
            ) : (
      ...
      {user.goals && user.goals.trim() !== "" && (
        <div className="bg-(--color-accent) border border-[#333333] rounded-xl overflow-hidden">
          <div className="p-5 border-b border-[#333333]">
            <h2 className="text-lg font-semibold text-(--color-secondary)">Your Goals</h2>
          </div>
          <div className="p-5">
