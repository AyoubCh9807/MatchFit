...
import ExpertDashboard from "../../../components/page";
      ...
      try {
        ...
        sessions?.forEach((s) => {
          if (s.status === "completed") {
            if (!trainerCounts[s.trainer_id])
              trainerCounts[s.trainer_id] = { count: 0, trainer: s.trainers };
            trainerCounts[s.trainer_id].count++;
          }
        });
        const top = Object.values(trainerCounts).sort(
          (a, b) => b.count - a.count
        )[0];
                  ...
                  >
                    <div>
                      <p className="font-semibold text-(--color-secondary)">
                        {s.trainers?.name || "Unknown Trainer"}
                      </p>
